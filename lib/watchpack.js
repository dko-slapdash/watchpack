...
class Watchpack extends EventEmitter {
	constructor(options) {
		...
		this.aggregateCondition =
			typeof options.aggregateTimeout === "function"
				? options.aggregateTimeout
				: () => true;
	...
	_scheduleAggregateTimer(delay = this.aggregateTimeout) {
		if (this.aggregateTimer) clearTimeout(this.aggregateTimer);
		this.aggregateTimer = setTimeout(this._onTimeout, delay);
	}

	_clearAggregateTimer() {
	...
	_onTimeout() {
		...
		const changes = this.aggregatedChanges;
		const removals = this.aggregatedRemovals;
		const res = this.aggregateCondition(changes, removals);
		const promise =
			res && typeof res.then === "function" ? res : { then: cb => cb(res) };
		promise.then(res => {
			if (res === true) {
				this.aggregatedChanges = new Set();
				this.aggregatedRemovals = new Set();
				...
				this._scheduleAggregateTimer(typeof res === "number" ? res : undefined);
			}
		});
	}
}
